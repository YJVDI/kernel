			 ...
			 struct ethtool_wolinfo *wol)
{
	...
	if (wol->wolopts & WAKE_MAGIC) {
		...
		wol_cfg.type = YTPHY_WOL_TYPE_LEVEL;
		wol_cfg.width = YTPHY_WOL_WIDTH_336MS;
...
static int ytphy_set_wol_init(struct phy_device *phydev,int EnableFlag)
{
	int ret, pre_page;
	ytphy_wol_cfg_t wol_cfg;
	struct net_device *p_attached_dev = phydev->attached_dev;
	...
	if (EnableFlag) {
        ...
        */
		...
		//ret = ytphy_wol_en_cfg(phydev, wol_cfg);
		ret = ytphy_write_ext(phydev, YTPHY_WOL_CFG_REG, 0x4d);
		if (ret < 0)
			return ret;
		...
		if (ret < 0)
			return ret;
		netdev_info(phydev->attached_dev, "yt8521 Set the WOL config Enable\n");
	} else {
		/*wol_cfg.enable = 0; //disable
		wol_cfg.type = YTPHY_WOL_TYPE_MAX;
		wol_cfg.width = YTPHY_WOL_WIDTH_MAX;
		ret = ytphy_wol_en_cfg(phydev, wol_cfg);
		*/
		ret = ytphy_write_ext(phydev, YTPHY_WOL_CFG_REG, 0x02);
		if (ret < 0)
			return ret;
		netdev_info(phydev->attached_dev, "yt8521 Set the WOL config Disable\n");
	}
...
static int yt8614_read_status(struct phy_device *phydev)
{
	...
	int port ;
	unsigned int base_phyaddr;
	port= yt8614_get_port_from_phydev(phydev);
	base_phyaddr= yt8614_chip_base_phy_get(phydev);
