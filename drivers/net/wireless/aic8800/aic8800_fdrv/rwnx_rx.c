			...
			if ((udph->source == __constant_htons(SERVER_PORT))
				...
				if (dhcph->cookie == htonl(DHCP_MAGIC) && dhcph->op == 2 &&
					...
					while (option[offset] != DHCP_OPTION_END && offset < length) {
						...
						if (rwnx_vif->sta.paired_cipher_type == WLAN_CIPHER_SUITE_CCMP || rwnx_vif->sta.paired_cipher_type == WLAN_CIPHER_SUITE_AES_CMAC || 
                                                ((rwnx_vif->sta.group_cipher_type == 0xff) && (rwnx_vif->sta.paired_cipher_type == 0xff)))
